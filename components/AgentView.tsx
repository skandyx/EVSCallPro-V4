        ...
        <div className="h-screen w-screen flex flex-col font-sans bg-slate-100 text-lg dark:bg-slate-900 dark:text-slate-200">
            ...
            <main className="flex-1 grid grid-cols-12 gap-4 p-4 overflow-hidden">
                <div className="col-span-3 flex flex-col gap-4">
                     <div className="bg-white dark:bg-slate-800 p-4 rounded-lg shadow-sm border dark:border-slate-700 flex-1 flex flex-col"><h2 className="text-xl font-semibold text-slate-800 dark:text-slate-100 border-b dark:border-slate-600 pb-2 mb-4">Informations</h2><div className="mb-4"><h3 className="text-base font-semibold text-slate-600 dark:text-slate-300">KPIs du Jour</h3>{agentState ? (<div className="grid grid-cols-2 gap-2 mt-2 text-center"><div className="bg-slate-100 dark:bg-slate-700 p-2 rounded-md"><p className="text-xs text-slate-500 dark:text-slate-400">Appels Traités</p><p className="text-xl font-bold text-slate-800 dark:text-slate-200">{agentState.callsHandledToday}</p></div><div className="bg-slate-100 dark:bg-slate-700 p-2 rounded-md"><p className="text-xs text-slate-500 dark:text-slate-400">DMT</p><p className="text-xl font-bold text-slate-800 dark:text-slate-200 font-mono">{formatTimer(agentState.averageHandlingTime)}</p></div></div>) : <p className="text-xs text-slate-400 italic mt-1">Chargement...</p>}</div><div className="flex-1 flex flex-col border-t dark:border-slate-600 pt-4"><h3 className="text-base font-semibold text-slate-600 dark:text-slate-300 mb-2 flex-shrink-0">Fiche Contact</h3><div className="flex-1 overflow-y-auto">{currentContact ? <div className="space-y-2 text-base"><p><span className="font-semibold">Campagne:</span> {currentCampaign?.name}</p><p><span className="font-semibold">Nom:</span> {currentContact.firstName} {currentContact.lastName}</p><p><span className="font-semibold">Code Postal:</span> {currentContact.postalCode}</p> {allPhoneNumbers.map(({name, number}) => (<div key={name} className="flex items-center justify-between"><p><span className="font-semibold">{name}:</span><span className="font-mono ml-2">{number}</span></p>{status === 'En Attente' && currentCampaign?.dialingMode === 'MANUAL' && <button onClick={() => handleDial(number)} className="bg-green-100 text-green-700 font-semibold py-1 px-3 rounded-md text-sm hover:bg-green-200">Appeler</button>}</div>))} </div> : <div className="h-full flex flex-col items-center justify-center text-center">{feedbackMessage ? <p className="text-amber-600 font-semibold">{feedbackMessage}</p> : <p className="text-slate-500 dark:text-slate-400">En attente d'un appel...</p>}{status === 'En Attente' && <button onClick={requestNextContact} disabled={isLoadingNextContact} className="mt-4 bg-primary text-primary-text font-bold py-2 px-4 rounded-lg shadow-md hover:bg-primary-hover disabled:opacity-50">{isLoadingNextContact ? 'Recherche...' : 'Prochain Contact'}</button>}</div>}</div></div></div>
