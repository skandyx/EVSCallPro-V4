...
interface AgentViewProps {
    ...
    // FIX: Changed the type of 'onStatusChange' to the full AgentStatus to allow for programmatic status changes like 'En Appel' or 'En Post-Appel', which were causing type errors.
...
const AgentView: React.FC<AgentViewProps> = ({ currentUser, onLogout, data, refreshData, onUpdatePassword, onUpdateProfilePicture, onUpdateContact, theme, setTheme, agentState, onStatusChange }) => {
    ...
    }, [onStatusChange, currentCampaign, requestNextContact]);
        ...
        <div className="h-screen w-screen flex flex-col font-sans bg-slate-100 text-lg dark:bg-slate-900 dark:text-slate-200">
            ...
            <main className="flex-1 grid grid-cols-12 gap-4 p-4 overflow-hidden">
                <div className="col-span-3 flex flex-col gap-4">
                    ...
                    <div className="bg-white dark:bg-slate-800 p-4 rounded-lg shadow-sm border dark:border-slate-700 flex-1 flex flex-col min-h-0"><div className="border-b dark:border-slate-600 pb-2 mb-2 flex items-center justify-between flex-shrink-0"><h2 className="text-xl font-semibold text-slate-800 dark:text-slate-100 flex items-center gap-2"><CalendarDaysIcon className="w-6 h-6 text-indigo-600 dark:text-indigo-400"/>{t('agentView.myCallbacks')}</h2><select value={callbackCampaignFilter} onChange={e => setCallbackCampaignFilter(e.target.value)} className="text-sm p-1 border bg-white dark:bg-slate-700 dark:border-slate-600 rounded-md"><option value="all">Toutes les campagnes</option>{assignedCampaigns.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div><div className="flex-1 overflow-y-auto pr-2 space-y-2 text-base">{mySortedCallbacks.length > 0 ? mySortedCallbacks.map(cb => {const now=new Date(); const scheduled=new Date(cb.scheduledTime); let itemClasses='w-full text-left p-3 rounded-md border-l-4 transition-colors disabled:opacity-60 disabled:cursor-not-allowed '; if(scheduled<now){const hoursDiff=(now.getTime()-scheduled.getTime())/(1000*60*60); if(hoursDiff > 24){itemClasses+='border-red-500 bg-red-50 dark:bg-red-900/20 hover:bg-red-100 dark:hover:bg-red-900/40';}else{itemClasses+='border-orange-500 bg-orange-50 dark:bg-orange-900/20 hover:bg-orange-100 dark:hover:bg-orange-900/40';}}else{const isToday=scheduled.getDate()===now.getDate() && scheduled.getMonth()===now.getMonth() && scheduled.getFullYear()===now.getFullYear(); if(isToday){itemClasses+='border-green-500 bg-green-50 dark:bg-green-900/20 hover:bg-green-100 dark:hover:bg-green-900/40';}else{itemClasses+='border-slate-300 bg-slate-50 dark:bg-slate-700 dark:border-slate-600 hover:bg-slate-100 dark:hover:bg-slate-600';}} const campaignName=data.campaigns.find(c=>c.id===cb.campaignId)?.name || 'Campagne inconnue'; return (<button key={cb.id} onClick={()=>handleCallbackClick(cb)} disabled={status !== 'En Attente'} className={itemClasses}><div className="flex justify-between items-baseline"><p className="font-semibold text-slate-800 dark:text-slate-200">{cb.contactName}</p><p className="text-xs text-slate-500 dark:text-slate-400">{campaignName}</p></div><p className="text-sm text-slate-600 dark:text-slate-400 font-mono">{cb.contactNumber}</p><p className="text-sm font-bold text-indigo-700 dark:text-indigo-400 mt-1">{scheduled.toLocaleString('fr-FR')}</p></button>);}) : (<p className="text-sm text-slate-500 dark:text-slate-400 text-center pt-8 italic">{t('agentView.noCallbacks')}</p>)}</div></div>
                </div>
