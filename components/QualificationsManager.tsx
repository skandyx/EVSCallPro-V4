...
const GroupEditModal: React.FC<GroupEditModalProps> = ({ group, allQualifications, onSave, onClose, onSaveQualification, onDeleteQualification }) => {
    ...
    const QualificationTree: React.FC<{
        ...
        if (nodes.length === 0 && level === 0) {
            return <p className="text-center text-sm text-slate-500 dark:text-slate-400 italic py-4">Aucune qualification.</p>;
        }
        return (
                            ...
                            <div className={`flex items-center justify-between p-2 bg-white dark:bg-slate-800 border dark:border-slate-700 rounded-md text-sm ${node.isStandard ? 'opacity-70' : 'hover:bg-slate-50 dark:hover:bg-slate-700'}`}>
                                ...
                                {!node.isStandard && (
                                    <div className="flex items-center gap-1">
                                        {!isAssignedList && <button type="button" onClick={() => {setEditingQual(node); setIsQualModalOpen(true)}} title="Modifier" className="p-1.5 rounded-full hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-500 hover:text-indigo-600 dark:hover:text-indigo-400"><EditIcon className="w-4 h-4"/></button>}
                                        {!isAssignedList && <button type="button" onClick={() => onDeleteQualification(node.id)} title="Supprimer" className="p-1.5 rounded-full hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-500 hover:text-red-600 dark:hover:text-red-400"><TrashIcon className="w-4 h-4"/></button>}
                                        <button type="button" onClick={() => isAssignedList ? handleUnassign(node.id) : handleAssign(node.id)} title={isAssignedList ? "Retirer" : "Assigner"} className="p-1.5 rounded-full bg-slate-200 dark:bg-slate-600 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 text-slate-600 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-400">
            ...
            <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-5xl h-[90vh] flex flex-col">
                <form onSubmit={handleSubmit} className="flex flex-col h-full">
                    <div className="p-6 border-b dark:border-slate-700 flex-shrink-0">
                        <h3 className="text-xl font-semibold leading-6 text-slate-900 dark:text-slate-100">{group ? 'Modifier le Groupe' : 'Nouveau Groupe'}</h3>
                        <div className="mt-4">
                            <label htmlFor="groupName" className="block text-sm font-medium text-slate-700 dark:text-slate-300">Nom du groupe</label>
                    ...
                    <div className="bg-slate-50 dark:bg-slate-900 px-6 py-4 flex justify-end gap-3 mt-auto flex-shrink-0 border-t dark:border-slate-700">
                        <button type="button" onClick={onClose} className="rounded-md border border-slate-300 bg-white px-4 py-2 font-medium text-slate-700 shadow-sm hover:bg-slate-50 dark:bg-slate-700 dark:text-slate-200 dark:border-slate-600 dark:hover:bg-slate-600">Annuler</button>
                        <button type="submit" className="rounded-md border border-transparent bg-primary px-4 py-2 font-medium text-primary-text shadow-sm hover:bg-primary-hover">Enregistrer</button>
                    </div>
                </form>
            </div>
        </div>
