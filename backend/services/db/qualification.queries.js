...
const saveQualification = async (q, id) => {
    ...
    if (id) {
        const res = await pool.query('UPDATE qualifications SET code=$1, description=$2, type=$3, parent_id=$4, is_recyclable=$5, updated_at=NOW() WHERE id=$6 AND is_standard = FALSE RETURNING *', [q.code, q.description, q.type, q.parentId, isRecyclable, id]);
        return keysToCamel(res.rows[0]);
    }
    const res = await pool.query('INSERT INTO qualifications (id, code, description, type, parent_id, is_recyclable, is_standard) VALUES ($1, $2, $3, $4, $5, $6, FALSE) RETURNING *', [q.id, q.code, q.description, q.type, q.parentId, isRecyclable, false]);
    return keysToCamel(res.rows[0]);
};
